...
class AmentGradleBuildTask(GradleBuildTask):
    """Build ROS packages with the build type 'ament_gradle'."""
    ...
    async def _build(self, args, env):
        ...
        args.gradle_args += [
            '-Pament.install_space={}'.format(args.install_base)]
        args.gradle_args += [
            '-Pament.dependencies={}'.format(ament_dependencies)]
        args.gradle_args += [
            '-Pament.package_manifest.name={}'.format(self.context.pkg.name)]
        args.gradle_args += [
            '-Pament.exec_dependency_paths_in_workspace={}'.format(
                ament_exec_dependency_paths_in_workspace)]
        # TODO(esteve): ament.gradle_recursive_dependencies is hardcoded
        #  to False for now
        args.gradle_args += [
            '-Pament.gradle_recursive_dependencies={}'.format(False)]
